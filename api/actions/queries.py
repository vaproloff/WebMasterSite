...
async def _get_urls_with_pagination_query(page, per_page, date_start, date_end, state, state_date, metric_type, session):
    async with session() as s:
        url_dal = QueryDAL(s)
        urls = await url_dal.get_urls_with_pagination(
            page, per_page, date_start, date_end, state, state_date, metric_type,
        )
        return urls


async def _get_urls_with_pagination_and_like_query(page, per_page, date_start, date_end, search_text, state, state_date, metric_type, session):
    async with session() as s:
        url_dal = QueryDAL(s)
        urls = await url_dal.get_urls_with_pagination_and_like(
            page, per_page, date_start, date_end, search_text, state, state_date, metric_type
        )
        return urls
